...
load_dotenv()
# Configuration
PI_STREAM_URL = os.getenv("PI_STREAM_URL", "http://192.168.X.X:5000/video_feed")
GO_BACKEND_URL = os.getenv("GO_BACKEND_URL", "ws://localhost:3000/ws/stream")
ALERT_COOLDOWN = float(os.getenv("ALERT_COOLDOWN", 2.0))


def get_mock_sensors():
    """Simulates real-time IoT sensor data for the MVP"""
    return {"temperature_c": 78, "gas_leak": True, "abnormal_sound": True}
    ...
    while True:
        ...
        if True:
            ...
            if current_time - last_alert_time >= ALERT_COOLDOWN:
                print(
                    f"🚨 THREAT DETECTED: {payload['situation']['hazard_type'].upper()}"
                )

                ret, buffer = cv2.imencode(
                    ".jpg", final_frame, [cv2.IMWRITE_JPEG_QUALITY, 80]
                )
                if not ret:
                    continue
